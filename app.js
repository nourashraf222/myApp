...
app.get('/wanttogobali',function(req,res){
  MongoClient.connect("mongodb://127.0.0.1:27017",function(err,client){
    ...
    db.collection(session.userid).find({}).toArray(function(err,results){
      if(results.length==0){
    db.createCollection(session.userid)
    db.collection(session.userid).insertOne({Destination:"Bali"});
      }
      else{
        db.collection(session.userid).find({Destination:'Bali'}).toArray(function(err,results){
          if (results.length==0)
          db.collection(session.userid).insertOne({Destination:"Bali"})
        })
      }
...
app.get('/wanttogoinca',function(req,res){
  MongoClient.connect("mongodb://127.0.0.1:27017",function(err,client){
    ...
    db.collection(session.userid).find({}).toArray(function(err,results){
      if(results.length==0){
    db.createCollection(session.userid)
    db.collection(session.userid).insertOne({Destination:"inca"});
      }
      else{
        db.collection(session.userid).find({Destination:'inca'}).toArray(function(err,results){
          if (results.length==0)
          db.collection(session.userid).insertOne({Destination:"inca"})
        })
      }
...
app.get('/wanttogoparis',function(req,res){
  MongoClient.connect("mongodb://127.0.0.1:27017",function(err,client){
    ...
    db.collection(session.userid).find({}).toArray(function(err,results){
      if(results.length==0){
    db.createCollection(session.userid)
    db.collection(session.userid).insertOne({Destination:"paris"});
      }
      else{
        db.collection(session.userid).find({Destination:'paris'}).toArray(function(err,results){
          if (results.length==0)
          db.collection(session.userid).insertOne({Destination:"paris"})
        })
      }
...
app.get('/wanttogorome',function(req,res){
  MongoClient.connect("mongodb://127.0.0.1:27017",function(err,client){
    ...
    db.collection(session.userid).find({}).toArray(function(err,results){
      if(results.length==0){
    db.createCollection(session.userid)
    db.collection(session.userid).insertOne({Destination:"rome"});
      }
      else{
        db.collection(session.userid).find({Destination:'rome'}).toArray(function(err,results){
          if (results.length==0)
          db.collection(session.userid).insertOne({Destination:"rome"})
        })
      }
...
app.get('/wanttogosantorini',function(req,res){
  MongoClient.connect("mongodb://127.0.0.1:27017",function(err,client){
    ...
    db.collection(session.userid).find({}).toArray(function(err,results){
      if(results.length==0){
    db.createCollection(session.userid)
    db.collection(session.userid).insertOne({Destination:"santorini"});
      }
      else{
        db.collection(session.userid).find({Destination:'santorini'}).toArray(function(err,results){
          if (results.length==0)
          db.collection(session.userid).insertOne({Destination:"santorini"})
        })
      }
...
app.get('/wanttogoannapurna',function(req,res){
  MongoClient.connect("mongodb://127.0.0.1:27017",function(err,client){
    ...
    db.collection(session.userid).find({}).toArray(function(err,results){
      if(results.length==0){
    db.createCollection(session.userid)
    db.collection(session.userid).insertOne({Destination:"annapurna"});
      }
      else{
        db.collection(session.userid).find({Destination:'annapurna'}).toArray(function(err,results){
          if (results.length==0)
          db.collection(session.userid).insertOne({Destination:"annapurna"})
        })
      }
